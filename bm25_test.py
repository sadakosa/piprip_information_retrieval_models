...
# MAIN BM25 FUNCTION
# ====================================================================================================
# def run_bm25(raw_papers_df, target_papers, title_weight, abstract_weight):
#     # Clean and tokenize the data
#     papers_df = tk.tokenise_papers_df(raw_papers_df)
#     save_to_csv(papers_df, "tokenised_text", "test")
...
#         # Run the query
#         title_title_scores = run_bm25_query(bm25_titles, target_paper[1]) # target title to other titles
#         title_abstract_scores = run_bm25_query(bm25_abstracts, target_paper[2]) # target title to other abstracts
#         abstract_title_scores = run_bm25_query(bm25_titles, target_paper[1]) # target abstract to other titles
#         abstract_abstract_scores = run_bm25_query(bm25_abstracts, target_paper[2]) # target abstract to other abstracts
...
#         papers_df['combined_score'] = combined_scores
#         ranked_papers_df = rank_papers(papers_df)
#         save_to_csv(ranked_papers_df, "results", "ranked_papers_two")

#         print("Top 5 papers:")
#         print(ranked_papers_df.head(5))
#     return
...
def run_bm25(db_client, raw_papers_df, target_ss_ids, title_weight, abstract_weight):
    ...
    bm25_titles = initialize_bm25(titles)
    bm25_abstracts = initialize_bm25(abstracts)
    # print("num raw papers: ", len(papers_df))

    overall_combined_scores = [0] * len(papers_df)

    target_papers = db_operations.get_papers_by_ss_ids(db_client, target_ss_ids)
    # print("num target papers: ", len(target_papers))
